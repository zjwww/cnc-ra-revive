...
**当前版本：v1.8。** 网站首页默认使用浏览器原生缩放。 提供两套缩放机制、四个独立文件。每个文件均保留原来的 3840 × 2160 坐标系、16:9 图像比例、177 个文字元素和固定内部布局。
...
四个文件均将手机双指缩放和平移交给浏览器。页面内缩放组仅在桌面继续使用代码处理滚轮与快捷键。手机标签页标题显示“手机浏览器手势缩放”，不显示容易误解的页面内倍率。
...
v1.7 移动端候选版已由用户在反馈的 iPhone 14 Pro／iOS 26.6.2 上试用，未发现新的问题。v1.8 保留该手势处理，改为按宽度适配；本次桌面及移动端自动化模拟不能替代新适配方式的真机测试。原图、箭头修复和 16:9 坐标保持不变。
...
GitHub Pages 继续使用浏览器原生缩放 HTML 作为首页，其余三个查看文件可通过上方链接访问。
