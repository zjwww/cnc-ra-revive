...
**当前版本：v1.5。** 网站首页默认使用浏览器原生缩放。 提供两套缩放机制、四个独立文件。每个文件均保留原来的 3840 × 2160 坐标系、16:9 图像比例、177 个文字元素和固定内部布局。
...
960 × 540 WebP 仅用于文档预览，不包含全尺寸 PNG。
...
当前仓库包含四个查看文件：`index.html`（原生 HTML）、`RA-4K.svg`（原生 SVG），以及 `RA-4K-viewer.html`、`RA-4K-viewer.svg`（页面内缩放）。原有首页与 SVG 地址继续有效。另保留小尺寸 WebP 缩略图及独立的中英文文档。当前 GitHub 文件树不包含历史版本目录或全尺寸 PNG；本地 v1.0～v1.5 存档全部保留。已发布版本由 Git 历史记录，本次静态网站更新不创建 GitHub Release。
...
GitHub Pages 使用 `main` 分支的仓库根目录。默认入口 `index.html` 是用户已实测确认的原生 HTML 查看器，与本地 v1.5 候选文件逐字节一致。两个 SVG 和另一套 HTML 可通过上方链接打开。GitHub 仓库首页默认展示 `README.md`；`.nojekyll` 使 Pages 直接发布静态文件。
